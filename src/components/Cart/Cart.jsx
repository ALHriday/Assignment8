...
const Cart = ({ isActive, productItem }) => {
    const {handleDeleteCartItem} = useContext(HandleContextApi);
    const { productTitle, price, productImage } = productItem || {};

    return (
        <div className={isActive.cart ? 'bg-slate-200' : 'bg-slate-200 hidden'}>
            
            <div className="grid grid-cols-4 border-b-2 border-black p-4 bg-green-300 w-11/12 mx-auto">
                ...
                <div className="p-4">
                    <button onClick={() => handleDeleteCartItem(productItem)} className="text-2xl btn btn-error">Del</button>
                </div>
            </div>
        </div>
    );
